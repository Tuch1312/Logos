...
public class GestioneLezioni {
	...
	public boolean creaLezioni(Corso c) {
		...
		c.setLeziones(new ArrayList<Lezione>());
	...
	public boolean modificaLezione(Docente d, Corso c, Lezione l){
		EntityManager em = JPAUtility.emf.createEntityManager();
		Lezione lezione = null;
		...
		if (c!=null && lezione!=null && d.getCodicePersona().equals(c.getDocente().getCodicePersona())) {
			l.setCorso(c);
			em.getTransaction().begin();
			em.merge(l);
			em.getTransaction().commit();
