...
public class GestionePersona {
	...
	public boolean registrazione(String nome, String cognome, String mail, String password, String immagine, String indirizzo, boolean isDocente) {
		...
		if(persona!=null) {
			...
			p.setCodicePersona(assegnaCodice(mail));
	...
	public String login(String mail, String psw) {
		...
		Persona persona = null;
		try {
		persona = em.find(Persona.class, assegnaCodice(mail));
		}catch(Exception e) {
			e.printStackTrace();
		}
...
//	}
		...
		public static String assegnaCodice(String mail) {
			  try {
		...
		public Persona getPersona(String codice) {
			EntityManager em = JPAUtility.emf.createEntityManager();
			return em.find(Persona.class, codice);
		}
